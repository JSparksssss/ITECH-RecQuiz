...
#对新建用户设置课程
def set_course(user_id,course_id):
    sql = 'insert into RecQuiz_course_user(course_id,user_id) values({},{})'.format(course_id,user_id)
    cursor = connection.cursor()
    cursor.execute(sql)
    connection.commit()
    print("set course success")
...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if user_form.is_valid() and profile_form.is_valid():
            ...
            uid = u.id
            courses = Course.objects.all()
            courselist = [x.id for x in courses]
            for course_id in courselist:
                set_course(uid,course_id)
        else:
