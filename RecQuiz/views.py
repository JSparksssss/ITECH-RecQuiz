...
def unbind(request,course_name_slug):

    context_dict = {}
    try:
        course_name = course_name_slug.split('/')[-1]
        print(course_name)
        print(request.path)
        # user = User.objects.get(slug=user_id_slug)
        # course = Course.objects.filter(User=user)
        ...
        print('222222')
        if user_slug:
            course_id = Course.objects.get(slug=course_name)
            course_id_str = course_id.id
            print(course_id_str)
            sql = "DELETE FROM RecQuiz_course_user WHERE course_id = \'" + str(course_id_str) + "\' and user_id = \'" + str(user_slug) + "\'"
            cursor = connection.cursor()
            cursor.execute(sql)
            flashsql = "SELECT course_id from RecQuiz_course_user where user_id=\'" + str(user_slug) + "\'"
            cursor = connection.cursor()
            cursor.execute(flashsql)
            results = cursor.fetchall()
            courses = []
            for course_id in results:
                print(course_id[0])
